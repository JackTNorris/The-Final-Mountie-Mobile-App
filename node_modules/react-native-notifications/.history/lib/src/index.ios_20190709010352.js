  ...
  static addEventListener(type: string, handler: Function) {
    if (_exportedEvents.indexOf(type) !== -1) {
      ...
      if (type === DEVICE_REMOTE_NOTIFICATIONS_REGISTERED_EVENT) {
        ...
        listener = DeviceEventEmitter.addListener(
          type,
          ({payload, identifier, action}) => handler(new IOSNotification(payload), (presentingOptions) => {
        ...
        listener = DeviceEventEmitter.addListener(
          type,
          ({payload, identifier, action}) => handler(new IOSNotification(payload), (presentingOptions) => {
            NativeRNNotifications.finishHandlingAction(identifier, presentingOptions);
          }, action)
        );
      }
