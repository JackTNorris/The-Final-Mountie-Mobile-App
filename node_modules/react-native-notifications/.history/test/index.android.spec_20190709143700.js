describe('Notifications-Android', () => {
  ...
  beforeEach(() => {
    ...
    deviceEventEmitterListenerStub = require('react-native').NativeModules.DeviceEventEmitter.addListener;
  ...
  describe('Registration token API', () => {
    it.only('should assign callback to native event upon listener registration', () => {
  ...
  describe('Notification token', () => {
    it('should refresh notification token upon refreshing request by the user', () => {
      expect(WixRNNotifications.refreshTokenStub).toHaveBeenCalledTimes(0);
      libUnderTest.NotificationsAndroid.refreshToken();
      expect(WixRNNotifications.refreshTokenStub).toHaveBeenCalledTimes(1);
    });
  });

  describe('Initial notification API', () => {
    it('should return initial notification data if available', (done) => {
      expect(WixRNNotifications.getInitialNotificationStub).toHaveBeenCalledTimes(0);
      const rawNotification = {foo: 'bar'};
      WixRNNotifications.getInitialNotificationStub.mockReturnValueOnce(Promise.resolve(rawNotification));
    ...
    it('should return empty notification if not available', (done) => {
      expect(WixRNNotifications.getInitialNotificationStub).toHaveBeenCalledTimes(0);
      WixRNNotifications.getInitialNotificationStub.mockReturnValueOnce(Promise.resolve(null));
  ...
  describe('Local notification', () => {
    ...
    it('should get published when posted manually', () => {
      expect(WixRNNotifications.postLocalNotificationStub).toHaveBeenCalledTimes(0);

      const id = libUnderTest.NotificationsAndroid.localNotification(notification);
      expect(id).toBeDefined();
      expect(WixRNNotifications.postLocalNotificationStub).toHaveBeenCalledWith(notification, id);
    });
    ...
    it('should be cancellable with an ID', () => {
      expect(WixRNNotifications.cancelLocalNotificationStub).toHaveBeenCalledTimes(0);

      libUnderTest.NotificationsAndroid.cancelLocalNotification(666);

      expect(WixRNNotifications.cancelLocalNotificationStub).toHaveBeenCalledWith(666);
    });
  });
});
